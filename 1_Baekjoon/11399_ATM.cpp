...
int main()
{
	int n, p[1000];	// n : the number of people, p : total waiting time
	int res = 0, total_time = 0;
	scanf("%d", &n);
	...
	sort(p, p+n);		// if 3rd parameter is default, then accending order.

	for (int i = 0; i < n; i++)
	{
		total_time += p[i];		// add to total_time at each case
		res += total_time;		// finally result 
	}
